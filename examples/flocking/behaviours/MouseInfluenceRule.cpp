...
Vector2 MouseInfluenceRule::computeForce(const std::vector<Boid*>& neighborhood, Boid* boid) {
    ...
    if (ImGui::IsMousePosValid() && io.MouseDown[0]) {
        ...
        Vector2 displacement = mousePos - boid->transform.position; // todo: change this
        float distance = Vector2::getDistance(mousePos, boid->transform.position); // todo: change this
